...
def filter_out_cases(df_event, out_events=None, reach_events=None):
    """
    아웃으로 끝나는 타석만 필터링
    ...
    # 아웃으로 끝나는 케이스만 필터링
    out_case_ids = case_results[case_results == 'out'].index.tolist()
    df_filtered = df_event[df_event['case_id'].isin(out_case_ids)].copy()
    
    return df_filtered, case_results.value_counts()


def filter_reach_cases(df_event, out_events=None, reach_events=None):
    """
    출루로 끝나는 타석만 필터링 (안타, 홈런 등)
    ...
    Returns:
        DataFrame: 출루로 끝나는 타석만 포함된 DataFrame
    """
    if out_events is None:
        out_events = ['strikeout', 'out', 'field_out', 'force_out', 'double_play', 'triple_play', 
                      'strikeout_double_play', 'sac_fly', 'sac_bunt']
    ...
    # 출루로 끝나는 케이스만 필터링
    reach_case_ids = case_results[case_results == 'reach'].index.tolist()
    df_filtered = df_event[df_event['case_id'].isin(reach_case_ids)].copy()
